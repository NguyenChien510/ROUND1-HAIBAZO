...
export default function App() {
  ...
  const [game, setGame] = useState({ circles: [], next: 1, time: 0, status: 'initial' })
  ...
  const start = () => {
    ...
    setGame({
      ...
      circles: Array.from({ length: num }, (_, i) => ({
        id: i + 1, x: Math.random() * (b.clientWidth - 50), y: Math.random() * (b.clientHeight - 50), cooldown: 0
      }))
    })
    timer.current = setInterval(() => {
      setGame(g => {
        ...
        if (won) { clearInterval(timer.current); setIsAuto(false); isAutoRef.current = false }

        // Handle Auto Play inside the same loop using the ref
        if (isAutoRef.current && !won) {
          autoTick.current++
          if (autoTick.current >= 10) { // Every 1s
            autoTick.current = 0
  ...
  const click = (id) => {
    ...
    setGame(g => {
      if (id !== g.next || g.circles.find(c => c.id === id).cooldown > 0) {
        if (id !== g.next) { clearInterval(timer.current); return { ...g, status: 'lost' } }
        return g
      }
  ...
  return (
    <div className="game-container">
      <h1 style={{ color: game.status === 'won' ? 'green' : game.status === 'lost' ? 'red' : '#000' }}>
        {game.status === 'won' ? 'ALL CLEARED' : game.status === 'lost' ? 'GAME OVER' : "LET'S PLAY"}
      </h1>
      <div className="controls">
        ...
        <div className="control-row">
          <button onClick={start}>{game.status === 'initial' ? 'Play' : 'Restart'}</button>
          <button onClick={() => {
      ...
      <div className="board" ref={boardRef}>
        {game.circles.map(c => (
          <div key={c.id} className="circle" onClick={() => click(c.id)} style={{
            ...
            background: c.cooldown > 0 ? '#ff4d4d' : '#fff', color: c.cooldown > 0 ? '#fff' : '#000'
          }}>
